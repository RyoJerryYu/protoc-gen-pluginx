...
func (g *TSRegistry) headerComments() string {
	lines := []string{
		fmt.Sprintf("// @generated by %s. DO NOT EDIT.", g.GenOpts.PluginName),
		"// versions:",
		fmt.Sprintf("// - %s %s", g.GenOpts.PluginName, g.GenOpts.VersionStr),
	}
	if g.GenOpts.F != nil {
		lines = append(lines, fmt.Sprintf("// source: %s", g.GenOpts.F.Desc.Path()))
	}
...
func (g *TSRegistry) Apply(w io.Writer) error {
	...
	content := g.buf.Bytes()
	header := g.headerComments()
	imports := g.ImportSegments()
	res := fmt.Sprintf("%s\n\n%s\n\n%s", header, imports, content)
